...
def build():
    ...
    '''cmds.addAttr(switch, ln = 'Clothing_Visibility', at = "enum", en = "____________", k = True)

    cmds.addAttr(switch, at = "bool", ln = 'Mittens_VIS', k = True, dv = True)

    cmds.addAttr(switch, at = "bool", ln = 'Shawl_VIS', k = True, dv = True)

    cmds.addAttr(switch, at = "bool", ln = 'Cape_VIS', k = True, dv = True)

    mitten = 'mittens_GRP'
    cape = 'cape_GEO'
    shawl = 'shawl_GEO'

    cmds.setDrivenKeyframe(f"{mitten}.visibility", cd = f"{switch}.Mittens_VIS", dv = 0, v = 0)
    cmds.setDrivenKeyframe(f"{mitten}.visibility", cd = f"{switch}.Mittens_VIS", dv = 1, v = 1)

    cmds.setDrivenKeyframe(f"{shawl}.visibility", cd = f"{switch}.Shawl_VIS", dv = 0, v = 0)
    cmds.setDrivenKeyframe(f"{shawl}.visibility", cd = f"{switch}.Shawl_VIS", dv = 1, v = 1)

    cmds.setDrivenKeyframe(f"{cape}.visibility", cd = f"{switch}.Cape_VIS", dv = 0, v = 0)
    cmds.setDrivenKeyframe(f"{cape}.visibility", cd = f"{switch}.Cape_VIS", dv = 1, v = 1)


    cmds.addAttr(switch, ln = 'Clothing_CTRL_Visibility', at = "enum", en = "____________", k = True)

    cmds.addAttr(switch, at = "bool", ln = 'Shawl_CTRL_VIS', k = True, dv = False)

    cmds.addAttr(switch, at = "bool", ln = 'Cape_CTRL_VIS', k = True, dv = False)

    capeCTRL = cmds.ls('*cape*JA*LOC')
    shawlCTRL = cmds.ls('*Shawl*JA*LOC')

    for ctrl in capeCTRL:
        cmds.setDrivenKeyframe(f"{ctrl}.visibility", cd = f"{switch}.Cape_CTRL_VIS", dv = 0, v = 0)
        cmds.setDrivenKeyframe(f"{ctrl}.visibility", cd = f"{switch}.Cape_CTRL_VIS", dv = 1, v = 1)

    for ctrl in shawlCTRL:
        cmds.setDrivenKeyframe(f"{ctrl}.visibility", cd = f"{switch}.Shawl_CTRL_VIS", dv = 0, v = 0)
        cmds.setDrivenKeyframe(f"{ctrl}.visibility", cd = f"{switch}.Shawl_CTRL_VIS", dv = 1, v = 1)'''
